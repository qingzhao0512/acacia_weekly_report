...
1. `antibotic_treatments_by_cases-2025-05-14.xlsx` 是ATC、基药、AWaRe分级的文档；基药会再用`essentialmedicine.xlsx`分析

1. `output_by_case_type.xlsx` 是徐老师让做的抗生素和其他药物联动分析
...
1. `formal_data_clean` 是基础数据；

1. `formal_data_final_modeimputed` 是在基础数据上进行多重插补的数据；

1. `excel_data` 是导入抗菌药物分布的数据；

1. `qualityresult` 是何文俊的数据
...
1. Firth’s penalized logistic regression和glm的family = binomial，不是一个东西对吧，另外firth不属于glm是吧
答：不是同一个东西：glm(family = binomial) 是 标准的广义线性模型（GLM），用于拟合二项分布的响应变量，通常使用 logit 链接函数。Firth’s penalized logistic regression 是一种 修正后的逻辑回归方法，在似然函数中加入惩罚项（penalization term），用来 减少偏倚，特别适用于小样本、稀有事件、完全或准完全分离（separation）等情况。

Firth方法不是标准GLM的一部分：Firth方法不能通过glm()函数直接实现，通常需要用专门的函数（如 R 中的 logistf()）。Firth回归用的是 惩罚的最大似然估计（penalized maximum likelihood estimation），而标准GLM用的是 最大似然估计（MLE）。


2. 即便抗生素处方开设率＞0.1，但是因为产后抑郁病例存在完全分离，因此即便可以用Poisson回归估计RR，也不能使用，只能用Firth logistic 回归
...
基药的文档，直接用：基药统计_20250619_111604
...
```{r}
# # 全国访问的数据
# url_value <- "https://www.wcrcnet.cn/redcap/api/"
# token_value <- "72020BE25F955E6D8E361C38FEFCF4D0"
# rcon <- redcapConnection(url = url_value,token = token_value)
# data_national_survey <- exportRecords(rcon,factors = FALSE)
# 
# data_national_survey$record_id <- as.character(data_national_survey$record_id)



load("data_national_survey_250513.rda")





# save(data_national_survey, file='data_national_survey_250513.rda')


# save(data_national_survey_replaced_add_r, file='data_national_survey_replaced_add_r.rda')
...
# 定义需要填充的列（排除 redcap_event_name 和药物相关列）
fill_cols <- setdiff(names(tmp), c("redcap_event_name", drug_cols))

# 按 record_id 分组，填充非药物列
...
# # 拆分数据并创建独立的数据框
# tmp %>%
#   inner_join(case_mapping, by = "visit_case") %>%  # 关联病例名
#   group_by(visit_case, case_name) %>%              # 按病例分组
#   group_split() %>%                                # 拆分成列表
#   walk(~{
...
#            envir = .GlobalEnv)
#     
#     message(sprintf("已创建数据框: tmp_%s (对应visit_case=%d)", 
#                    case_name, case_num))
#   })
...
用`antibotic_drug_info_by_cases-2025-05-14.xlsx`进行分析；
...
用`antibotic_treatments_by_cases-2025-05-14.xlsx`进行分析
...
`data_analysis`的`record_id`，会比`unique1_id_df`少，因为前者有的id并没有填写`afterclinic_sp_arm_1`。打开`gs_visit_7_r`和`gs_visit_114_r`查看，都没有成功访问。

所以最后纳入分析的，应该就是`data_analysis`的`record_id`。
...
data_analysis <- data_analysis %>%
        dplyr::select( record_id, visit_case, sp_name, investigator_name,        # 单个变量
                institution_name_register:visit_doctor,                   # 变量范围
...
data_analysis <- data_analysis %>%
  mutate(
    case_name = case_when(
      ...
      visit_case == 8 ~ "心绞痛",
      visit_case == 9 ~ "腰背痛",
      visit_case == 10 ~ "压力性尿失禁",
      visit_case == 11 ~ "高血压",
      visit_case == 12 ~ "糖尿病",
      TRUE ~ NA_character_  # 处理未匹配的情况
    ),
    .after = visit_case  # 放在visit_case列后面
  )

data_analysis <- data_analysis %>%
  mutate(
    # 从 record_id 提取省份缩写（前2或3个字母，不区分大小写）
    province_abbr = str_extract(record_id, "^[A-Za-z]{2,3}") %>% toupper(),  # 转为大写统一格式
    
    # 根据省份缩写映射为中文名称
    province_name = case_when(
      province_abbr %in% c("NMG") ~ "内蒙古自治区",
      province_abbr %in% c("HN") ~ "湖南省",
      province_abbr %in% c("GD") ~ "广东省",
      province_abbr %in% c("SC") ~ "四川省",
      province_abbr %in% c("GZ") ~ "贵州省",
      province_abbr %in% c("SX") ~ "陕西省",
      province_abbr %in% c("GS") ~ "甘肃省",
      TRUE ~ NA_character_  # 未匹配的设为NA
    ),
    .after = record_id  # 将 province_name 放在 record_id 列后
  )
...
这里是我整理的除了药物和处置的其他所有原始变量，再和何文俊的`ACACIA-qualityresult-hwj-20231112.xlsx`结合，并且删除对应原始数据就好

何文俊说最后纳入分析的数据量：`2143次`
...
qualityresult <- import("ACACIA-qualityresult-hwj-20231112.xlsx", which = "赵青清洗后用来分析的数据")

data_analysis_raw <- data_analysis



# 删除何文俊整理的最终数据，所对应的原始数据
data_analysis <- data_analysis %>% 
...
# 找出在 qualityresult 但不在 data_analysis 中的 record_id——值为0，可以纳入分析
...
tmp <- qualityresult %>%
  left_join(data_analysis, by = "record_id") %>% 
...
# 1. 读取antibiotic数据——每次更改excel，都需要把前面的all重新改一下
antibiotic_data <- read_excel("antibotic_treatments_by_cases-2025-05-14.xlsx", 
                            sheet = "all") %>%
  # 关键步骤：对record_id去重（保留每个ID的第一条记录）
...
# 验证结果
final_data %>%
  count(prescribe_all_antibiotic)  # 检查开药数量分布
...
prescribe_all_antibiotic：指的是开了几种抗生素
...
## diagnosis_quality 设为分类因子
# 合并“完全正确”和“基本正确”为“正确”
final_data$诊断质量 <- as.character(final_data$诊断质量)
final_data$诊断质量[final_data$诊断质量 %in% c("部分正确", "基本正确")] <- "部分正确"


final_data$诊断质量 <- factor(final_data$诊断质量,
         levels = c("完全正确", "部分正确", "错误", "没有下诊断")
)
...
final_data$doctor_title <- factor(
  ...
  levels = c(1, 2, 3, 4, 5),
  labels = c("无职称", "住院医师", "主治医师", "副主任医师", "主任医师"),
  ordered = TRUE
)


## doctor_education设为有序因子
# 将“无法获取”编码为 NA
...
final_data$doctor_education <- factor(
  ...
  levels = c(1, 2, 3, 4, 5),
  labels = c("中专", "高中及以下", "大专", "大学本科", "研究生"),
  ordered = TRUE
)


## doctor_certificate设为有序因子
# 将“无法获取”编码为 NA
...
final_data$doctor_certificate <- factor(
  ...
  levels = c(1, 2, 3, 4),
  labels = c("无证书", "乡村医生", "执业助理医师", "执业医师"),
  ordered = TRUE
)


## 创建 ownership_type 二元分类
final_data <- final_data %>%
  # 创建 ownership_type 二元变量（11/12为"公立"，其他为"私立"）
  mutate(
    ownership_type = ifelse(ownership %in% c(11, 12), "公立", "私立"),
    ownership_type = factor(ownership_type, levels = c("私立", "公立"))
  ) %>%
...
## 创建 institution_type 五元分类
final_data <- final_data %>%
  mutate(
    institution_type_group = case_when(
      institution_type %in% c("A100", "A210", "A300") ~ "医院",
      institution_type %in% c("B100", "B200") ~ "社区卫生中心（站）",
      institution_type %in% c("C210", "C220") ~ "乡镇卫生院",
      institution_type %in% c("D110", "D121", "D122", "D130", "D211", "D212", "D213", "D229") ~ "诊所",
      institution_type == "D600" ~ "村卫生室",
      TRUE ~ NA_character_  # 其他未分类的设为 NA
    ),
    institution_type_group = factor(institution_type_group,
                                    levels = c("医院", "社区卫生中心（站）", "乡镇卫生院", "诊所", "村卫生室")) 
  )
...
final_data <- final_data %>%
  mutate(
    institution_type_houyu = case_when(
      institution_type %in% c("A100", "A210", "A300") ~ "医院",
      institution_type %in% c("B100") ~ "社区卫生服务中心",
      institution_type %in% c("B200") ~ "社区卫生服务站",
      institution_type %in% c("C210", "C220") ~ "乡镇卫生院",
      institution_type %in% c("D110", "D121", "D122", "D130") ~ "门诊部",
      institution_type %in% c("D211", "D212", "D213", "D229") ~ "诊所",
      institution_type %in% c("D600") ~ "村卫生室",
      TRUE ~ NA_character_  # 其他未分类的设为 NA
    ),
    institution_type_houyu = factor(institution_type_houyu,
                                    levels = c("医院", "社区卫生服务中心", "社区卫生服务站", "乡镇卫生院", "诊所", "村卫生室")) 
  )
...
# 将四个费用变量中等于0的值替换为NA
final_data$`问诊费`[final_data$`问诊费` == 0] <- NA
final_data$`检查费`[final_data$`检查费` == 0] <- NA
final_data$`处置费`[final_data$`处置费` == 0] <- NA
final_data$`总费用`[final_data$`总费用` == 0] <- NA
...
250720：宋再伟老师建议`非营利`和`营利`可以不纳入分析
...
1. 无序分类变量需要转为factor，比如数据中是字符型（character）或用数字编码（如 1=男, 2=女），如果不转为 factor，模型会：错误地把它们当成连续变量进行线性建模；

1. 有序分类变量需要转为 ordered factor；
...
在 R 中使用 GLMM（如 glmer()）建模时，有序因子默认的“第一个水平”是参考组（baseline）；



`record_id`

391	gd_visit_524	 ——东平镇东方红卫生站	           ——应该是1，非营利性	
1845	sc_visit_2160	 ——大竹县欧家乡普山村卫生站        ——应该是1，非营利性		
2024	sx_visit_1504    ——西安市临潼区凤凰社区卫生服务中心——应该是1，非营利性
...
formal_data <- final_data %>%
  dplyr::select(
    ...
    # 因变量
    antibiotic_prescribed_num = prescribe_all_antibiotic,  # 重命名为更清晰的变量名
    ...
    # 医疗质量相关变量
    diagnosis_quality = 诊断质量,
    history_taking_ratio = `推荐问诊中实际问诊的比例`,  # 避免使用中文变量名
    physical_exam_ratio = `推荐体格检查中实际执行的比例`,
    lab_imaging_ratio = `推荐实验室或影像检查中实际建议的比例`,
    PPPC,
...
缺失率（Missing Rate）	建议处理方式
0–5%	可忽略或直接插补（影响极小）
5–20%	推荐使用 MICE 等方法进行插补
20–30%	可用 MICE 插补，但需谨慎，特别是变量重要时
>30%	通常建议剔除（除非该变量非常关键）


treatment_fee：处置费
examination_fee：检查费
doctor_education：医生教育程度
member_alliance：是否是某个医联体或医共体的成员；
member_corporate：是否属于⼀个医疗集团/连锁机构；
univ_affiliation：是否是⼤学附属医疗机构；
ins_contractor：是否是医保定点机构；
consultation_fee：问诊费
doctor_certificate：医生证书类别；
hospital_class：医院等次；
hospital_level：医院级别；
doctor_title：医生职称
total_fee：总体费用
...
# 连接USP性别和年龄，并修改sp_age的三等级
supplemental_data <- read_csv("formal_data_clean_with_sp_name.csv") %>%
  dplyr::select(record_id, sp_gender, sp_age)  # 只保留需要的列


# 合并数据，确保只保留formal_data_clean中的record_id
...
formal_data_clean <- formal_data_clean %>%
  ...
  mutate(sp_age_group = factor(sp_age_group, levels = c("≤30", "30–49", "≥50")))
...
# 方法1：使用 forcats::fct_recode()（最简洁）
formal_data_clean <- formal_data_clean %>%
  mutate(
    sp_gender = fct_recode(sp_gender, NULL = "#N/A")  # 将 "#N/A" 转为 NA
  )




# 发现 doctor_ethnic 变量中 "9" 取值
...
for (var in factor_vars) {
  ...
  df <- data.frame(
    类别 = names(tab),
    频数 = as.vector(tab),
    百分比 = round(100 * as.vector(prop), 1)
  )
  print(df)
}
...
`antibiotic_prescribed` 百分比是9.5%，可以用OR值
...
连续变量间：相关系数 `|r| > 0.8` 可视为存在较强共线性。

`total_fee` 和 `medication_fee` 相关性很大
...
对于多类别的分类变量，**只输出总的 GVIF（以及其调整值）**是推荐做法，不需要每个虚拟变量都单独输出 VIF。——注意，这个是和管老师那篇文章不一样的地方



**计算分类变量的方差膨胀因子（VIF），是在建立glmm模型时进行吗?**

VIF 的计算建议在 GLMM 建立之前进行，而不是在建立 GLMM 时进行。
原因如下：

1. VIF 是评估固定效应变量之间的多重共线性工具，本质上是通过拟合一个普通的线性（或广义线性）模型计算出来的；

1. GLMM 引入了随机效应（如 (1 | province_id)）之后，模型结构变复杂，VIF 的定义和计算就不再直接适用；

1. 因此，推荐在建立 GLMM 之前，使用一个普通的逻辑回归模型（如 glm()）进行 VIF 检查。


**结果解读**
1. 没有变量 GVIF^(1/(2*Df)) 5
   - 通常我们看的是最后一列 GVIF^(1/(2*Df))，它标准化了解释变量的自由度。
   - 实务中若该值 > 5（有些研究更严一点用 2 或 3），说明共线性严重。

1. 你的所有变量都 < 3，大多数 < 1.3，说明：
   - 💡当前固定效应变量之间的多重共线性问题很轻微，无需删除或合并变量。
...
formal_data_final_modeimputed <- formal_data_clean %>%
  mutate(
    # 分类变量：保持类型一致，使用 coalesce + get_mode 更稳妥
...
# 设置参考组
formal_data_final_modeimputed$case_type <- relevel(factor(formal_data_final_modeimputed$case_type), ref = "偏头痛")
...
# 所有自变量（注意替换 sp_age 为 sp_age_group）
all_vars <- c(
...
数据框里，estimate → 就是 OR
最终文档见`univariate_logistic_results_20250619_151247`
...
1. 筛选与结局变量（比如 antibiotic_prescribed）显著相关的因素，为后续多因素分析做准备；

2. 单因素分析时，是把11个病例放入，还是什么？
答：R 会自动处理这个因子变量，把其中 1 类设为参考组，另外 10 类分别与参考组比较，输出每一类的 OR 和 P 值。

3. 另外，11个病例如果只有几个有显著意义，那怎么处理？
答：把整个变量 case_type 放入多因素模型中，不管具体哪个类显著、哪个不显著。


这儿只有waiting time，PPPC还有doctor ethnic是需要删除的，但是后两者是医生的个人特征，还是先纳入吧
...
### 构建glm模型，二分类变量——不用这个代码，用下面的Firth 惩罚逻辑回归
medication_fee、other_fee没有放
...
case_type各分类的OR值异常高（如普通感冒的OR=2.8亿）,当某些病例类型（如"普通感冒"）的所有患者要么全部开抗生素，要么全部不开抗生素时，会导致逻辑回归系数无限大（OR值爆炸）。需要使用 Firth校正逻辑回归 解决分离问题：

另外，关于用RR还是OR值：
1. 优先使用 OR 的情况
✅ 研究目标为探索性分析（如变量筛选）
✅ 存在罕见事件（<10%）或数据分离（如某些病例类型100%开抗生素）
✅ 需要与既往文献保持一致（多数病例对照研究默认报告OR）

1. 优先使用 RR 的情况
✅ 研究目标为临床决策支持（需直接解释风险变化）
✅ 发生率接近10%阈值（如你的9.5%）且模型收敛良好
✅ 读者群体更熟悉RR（如公共卫生或政策研究）



以上全失效，应该用偏头痛病例作为reference组——结论可以用：

1. case_type普通感冒
2. case_type胃炎
3. case_type哮喘
4. diagnosis_quality错误
5. lab_imaging_ratio
6. case_type压力性尿失禁
7. rural_typerural
8. consultation_time
9. diagnosis_quality部分正确	
...
### 使用 Firth校正逻辑回归 解决分离问题：必须要用Firth校正，因为产后抑郁抗生素数量是0——
```{r}
...
# 
# ### 变量2：发现赵厚宇老师建议的结果，跑出来之后主体研究和敏感性分析结果相差很大。最终去除tm，加上机构分类，去除营利和非营利
# firth_model <- logistf(
#   antibiotic_prescribed ~ 
#     case_type +
#     consultation_time +
...
### 变量3：发现变量2跑出来之后主体研究和敏感性分析结果相差很大。最终去除tm，去除机构分类，去除营利和非营利
firth_model <- logistf(
  antibiotic_prescribed ~ 
    case_type +
    consultation_time +
...
# 提取置信区间，使用 confint()，并转换为 OR 范围，保留3位小数
...
OR > 1：该变量水平越高，更有可能发生因变量为 1（如开抗生素）

OR < 1：该变量水平越高，更不可能发生因变量为 1

OR = 1：该变量与因变量没有关联

lab_imaging_ratio：OR = 0.39，p = 0.0069
每单位提升 lab_imaging_ratio，处方抗生素的可能性下降了约 61%（1 - 0.39）

置信区间（0.18–0.78）不包含 1，说明显著相关

👉 解释：检查率越高，越不容易开抗生素，可能因为诊断更明确了。
...
1. case_type哮喘	
1. lab_imaging_ratio
1. diagnosis_quality错误
1. case_type产后抑郁	
1. rural_typerural
1. case_type压力性尿失禁
1. consultation_time
1. diagnosis_quality部分正确





### 构建glm模型，连续型变量——判断敏感性——原版
```{r}
...
最终文档见`robustness_univ_results.xlsx`
...
```
最后输出结果见`multi_results2-2025-06-26`





### 构建glm模型，连续型变量——判断敏感性——赵厚宇老师建议把数据剔除产后抑郁的再跑
```{r}
...
### 发现赵厚宇老师建议的结果，跑出来之后主体研究和敏感性分析结果相差很大。最终去除tm，加上机构分类，去除营利和非营利
# # 最终变量列表（含强制纳入 + p<0.2）
...
### 发现赵厚宇老师建议的结果，跑出来之后主体研究和敏感性分析结果相差很大。最终去除tm，去除机构分类，去除营利和非营利
final_vars <- c("province", "case_type", "diagnosis_quality",
              "history_taking_ratio", "physical_exam_ratio", "lab_imaging_ratio",
              "doctor_age", "doctor_ethnic", "doctor_female", 
...
### 发现赵厚宇老师建议的结果，跑出来之后主体研究和敏感性分析结果相差很大。最终去除tm，加上机构分类，去除营利和非营利/三者全去除
formal_data_final_modeimputed <- formal_data_final_modeimputed %>%
  mutate(antibiotic_prescribed = as.numeric(as.character(antibiotic_prescribed)))
...
```
最后输出结果见`multi_results_houyu-2025-07-20`
...
### 构建glm模型：原始数据——比连续型变量区别更大，还是就用连续型变量吧
#### 先单因素再多因素
...
# 设定参考水平（可选，如你需要）
formal_data_clean$case_type <- relevel(factor(formal_data_clean$case_type), ref = "偏头痛")
...
case_type小儿腹泻	case_type小儿腹泻	0.165	0.030	0.876	0.037
diagnosis_quality错误	diagnosis_quality错误	1.806	0.995	3.311	0.053




### 构建中介模型：问诊时长 诊断正确率 抗生素处置正确率。
```{r}

# diagnosis_binary: 正确诊断 = 1，其它 = 0
formal_data_final_modeimputed$diagnosis_binary <- ifelse(
  formal_data_final_modeimputed$diagnosis_quality %in% c("完全正确"),
  1, 0
)


# 二值化中介变量 diagnosis_quality，正确=1，其余=0——这行代码其实没用上
formal_data_final_modeimputed$diagnosis_quality <- ifelse(
  formal_data_final_modeimputed$diagnosis_quality %in% c("完全正确", "基本正确"), 1, 0
)
...
# 结果模型：问诊时长 + 诊断正确性 → 抗生素处置是否正确（Y ~ X + M）
model.Y <- glm(
...
set.seed(1234)  # 可复现的结果
med.out <- mediate(
  model.m = model.M,
  model.y = model.Y,
  treat = "consultation_time",     # 自变量
  mediator = "diagnosis_binary",  # 中介变量
  boot = TRUE,
  sims = 1000
)
...
## Total Effect 没有输出95%CI，下面自己构建

# 提取 bootstrap 样本
acme_samples <- med.out$d.avg.sim
ade_samples <- med.out$z.avg.sim

# 逐一相加得到 Total Effect 的 bootstrap 样本
...
1. ACME（Average Causal Mediation Effect）= -0.000462，p = 0.89
表示通过中介变量 diagnosis_correct（诊断是否正确）的间接效应非常小且不显著。

即：consultation_time 对 antibiotic_prescribed 的影响并不是通过“诊断是否正确”这个路径起作用的。


2. ADE（Average Direct Effect）= 0.002043，p < 0.0000000000000002*
表示 consultation_time 对 antibiotic_prescribed 的直接效应显著为正。

即：医生在诊室停留时间越长，直接降低处方抗生素的概率（因为因变量是“是否开抗生素”，数值越小表示越少处方抗生素）。



3. Total Effect（总效应）= 0.001581，p = 0.02*
表明 consultation_time 对 antibiotic_prescribed 的总效应也是显著的。


4. Proportion Mediated（间接路径所占比例）≈ -29%，p = 0.91
虽然值是负的（可能因间接效应方向相反），但p 值非常不显著。

表明“诊断是否正确”这一中介路径基本没有中介作用。


本研究采用 非参数Bootstrap 方法进行中介分析，结果显示，“诊室时间”对“不合理使用抗生素”的影响主要通过直接路径产生（直接效应 ADE = 0.0020，p < 0.001），而非通过“诊断是否正确”这一中介变量（间接效应 ACME = -0.00046，p = 0.89）。间接路径不显著，说明提高诊断正确性并非“诊室时间”影响处方行为的主要机制。


如何汇报：

在中介分析的论文写作中，一般只报告 average（平均）值，不用汇报control/treated。这是大多数医学、公共卫生和社会科学期刊的惯例
...
### 构建链式中介模型（陈江芸老师指导）：问诊时长 检查情况 诊断正确率 抗生素处置正确率。
#### history_taking_ratio作为M1
```{r}
...
检验中介模型中的系数 c'，如果不显著,即直接效应不显著，说明只有中介效应
...
### physical_exam_ratio作为M1，diagnosis_quality作为M2
...
偏头痛只有一个取值：lab_imaging_ratio
产后抑郁只有一个取值："physical_exam_ratio，lab_imaging_ratio
腰背痛：sp_gender, lab_imaging_ratio
哮喘：sp_gender
心绞痛：sp_gender
压力性尿失禁：sp_gender
高血压：无
...
#### 偏头痛——没开抗生素，所以没法分析
#### 产后抑郁——没开抗生素，所以没法分析
#### 小儿腹泻——开了，但是不收敛，所以要用Firth 惩罚逻辑回归：全都没有统计学效果
##### 先单因素再多因素
```{r}
...
univ_table <- data.frame(
  Variable = all_vars,
  P_value = round(unlist(univ_results), 4)  # 保留四位小数
)
...
ci <- exp(confint(firth_model))                        # 置信区间
p <- firth_model$prob                                  # P 值
...
#### 普通感冒——开了，影响因素：diagnosis_quality错误
##### 先单因素再多因素
```{r}
...
#### 哮喘——开了：doctor_ethnic2完全分离，所以要用Firth 惩罚逻辑回归
##### 先单因素再多因素
```{r}
...
firth_model <- logistf(
  ...
  data = migraine_data,
  pl = TRUE  # 启用精确 profile likelihood CI（默认建议启用）
)

# 提取 OR 和 CI
or <- round(exp(firth_model$coefficients), 3)
ci <- round(exp(confint(firth_model)), 3)  # logistf 自带 confint 方法

# 提取 P 值，科学计数法显示 3 位小数
p_values <- format.pval(firth_model$prob, digits = 3, eps = 0.001)
...
#### 心绞痛——开了：doctor_age完全分离，所以要用Firth惩罚逻辑回归
##### 先单因素再多因素
```{r}
...
#### 腰背痛——开了：doctor_age，doctor_ethnic2完全分离，所以要用Firth惩罚逻辑回归
##### 先单因素再多因素
```{r}
...
#### 压力性尿失禁——开了：没有sp_gender。没有影响因素
##### 先单因素再多因素——单因素也没有统计学意义
```{r}
...
#### 高血压——开了：doctor_age,doctor_ethnic完全分离，所以要用Firth惩罚逻辑回归。没有影响因素
##### 先单因素再多因素
```{r}
...
#### 糖尿病——开了：doctor_age,doctor_ethnic完全分离，所以要用Firth惩罚逻辑回归。没有影响因素
##### 先单因素再多因素
```{r}
...
### 分系统构建glm模型
1. 神经系统
偏头痛（1）：与大脑神经和血管功能异常相关。

产后抑郁（2）：属于精神心理障碍，与神经递质失衡有关。

2. 消化系统
⼩⼉腹泻（3）：通常由肠道感染或消化功能紊乱引起。

胃炎（6）：胃黏膜的炎症，属于消化系统疾病。
...
⾼⾎压（11）：涉及血管压力异常，属于心血管疾病。

5. 内分泌系统
糖尿病（12）：胰岛素分泌或作用异常导致的代谢性疾病。

6. 泌尿系统
压⼒性尿失禁（10）：与膀胱和尿道控制功能异常相关。

7. 运动系统（肌肉骨骼系统）
腰背痛（9）：可能由肌肉、骨骼或神经问题引起，如腰椎间盘突出、肌肉拉伤等。
...
# 输出结果
cat("只在 Excel 中的 record_id 数量：", length(only_in_excel), "\n")
print(only_in_excel)

cat("只在数据框 formal_data_final_modeimputed 中的 record_id 数量：", length(only_in_data), "\n")
print(only_in_data)
...
prescription_rate_data %>%
  ...
  kable(digits = 3, caption = "表1：全样本抗生素处方率") %>%
...
# 所有病例类型
all_case_types <- c(
  "偏头痛", "压力性尿失禁", "哮喘", "小儿腹泻", "心绞痛",
  "普通感冒", "糖尿病", "胃炎", "腰背痛", "高血压", "产后抑郁"
)
...
total_summary <- tibble(
  特征类型 = "总体汇总",
  特征名称 = "全部样本",
  总样本数 = paste0(total_n, " (100%)"),
  开抗生素数 = paste0(abx_n, " (", sprintf("%.2f", abx_n / total_n * 100), "%)")
)
...
# 统一空字段列
fill_cols <- setdiff(names(total_summary), c("特征类型", "特征名称"))
...
for (group_label in names(group_vars)) {
  ...
  combined <- combined %>%
    ...
    mutate(
      ...
      p值 = formatted_p,
      显著性 = sig_label,
      检验方法 = chisq_result$method,
      统计值 = chisq_result$statistic
    ) %>%
    rename(特征名称 = group) %>%
    dplyr::select(特征类型, 特征名称, everything())
...
# 保存文件
output_filename_csv <- paste0("抗生素处方统计表-", format(Sys.Date(), "%Y%m%d"), ".csv")
write.csv(final_table_with_total, file = output_filename_csv, row.names = FALSE)

cat("✅ 导出完成，文件名为：", output_filename_csv, "\n")
...
注意输出的表，doctor_female1和doctor_age是有缺失值的，缺4个
...
# 构建 Overall Summary 行
total_summary <- tibble(
  特征类型 = "总体汇总",
  特征名称 = "全部样本",
  总样本数 = paste0(total_n, " (100%)"),
  开抗生素数 = paste0(abx_n, " (", round(abx_n / total_n * 100, 1), "%)")
)
...
for (group_label in names(group_vars)) {
  ...
  case_cols <- setdiff(names(combined), c("group", "total_n", "abx_n", "总样本数", "开抗生素数"))
  ...
  combined <- combined %>%
    ...
    rename(特征名称 = group) %>%
    dplyr::select(特征类型, 特征名称, dplyr::everything())
...
# 输出文件名
output_filename_csv <- paste0("抗生素处方统计表0623-", format(Sys.Date(), "%Y%m%d"), ".csv")
...
# 找出province、rural_type 或 management_type 缺失的记录
library(dplyr)
...
prescription_rate_data %>%
  ...
  kable(digits = 3, caption = "表4：按疾病类型分层抗生素处方率") %>%
  kable_styling(full_width = FALSE)
...
abx_all %>%
  ...
  kable(digits = 3, caption = "表5a：抗菌药物基药目录分布") %>%
  kable_styling(full_width = FALSE)
...
两个是NA，因为药品名称只是记录为头孢，没法判断具体是哪个药品，所以基药设置为`NA`。
...
abx_all %>%
  mutate(AWaRe = str_to_title(AWaRe)) %>%  # 统一成 Access/Watch/Reserve
  group_by(AWaRe) %>%
  summarise(count = n(), .groups = "drop") %>%
  mutate(percentage = count / sum(count)) %>%
  kable(digits = 3, caption = "表5b：抗菌药物AWaRe目录分布") %>%
  kable_styling(full_width = FALSE)




# 合并 antibiotic_1 和 antibiotic_2 的 AWaRe 分类及 record_id
...
#### Table 3 AWare分类（备选，因为不同特征有缺失值，有缺失值的地方数值是0）
...
generate_aware_table <- function(data, case) {
  ...
  # 合并 antibiotic_1 和 antibiotic_2
  aware_1 <- data_case %>%
    filter(!is.na(antibiotic_1)) %>%
    mutate(AWaRe = AWaRe_1) %>%
  ...
  if (nrow(aware_all) == 0) {
    message(paste0("Case '", case, "' 中无有效数据，跳过生成表格"))
    return(NULL)
  }
  ...
  # 命名列
  colnames(result_df) <- c("特征", "水平", "Access", "Watch", "Reserve")

  # 强制保留所有特征水平
  result_df <- feature_level_full %>%
    left_join(result_df, by = c("特征", "水平")) %>%
  ...
  aware_summary <- aware_summary %>% dplyr::select(特征, 水平, all_of(all_aware_levels))
...
message(paste0("所有表格已保存到 ", filename))
...
##### 转成横向堆积图：Fugure 1. Distribution of prescribed antibiotic by WHO AWaRe categories for 11 case at primary healthcare
...
# # 原始数据
# data <- data.frame(
#   case_type_cn = c("偏头痛", "压力性尿失禁", "哮喘", "小儿腹泻", "心绞痛", "普通感冒", 
#                    "糖尿病", "胃炎", "腰背痛", "高血压"),
...
#   scale_fill_manual(values = custom_colors, breaks = c("Watch", "Access")) +  # breaks确定图例顺序
...
# Step 2: 各 case_type 总抗生素数
case_totals <- atc_long %>%
  count(case_type, name = "total")

# Step 3: 各 ATC × case_type 的频数 + 比例
...
total_row <- case_totals %>%
  ...
  mutate(
    ATC = "Total",
    总计 = paste0(sum(case_totals$total), " (100.0%)")
  ) %>%
  dplyr::select(ATC, 总计, everything())
...
# 统计每个 case_type 和 essential 的数量
...
summary_total <- count_df %>%
  ...
  mutate(
    percent = sprintf("%.1f%%", count / total * 100),
    总计 = paste0(count, " (", percent, ")")
  ) %>%
  dplyr::select(essential, 总计)
...
wide_df <- count_df %>%
  ...
  relocate(总计, .after = essential) %>%  # 将“总计”列放在病例最前面
  rename(`基药使用情况` = essential)
...
total_row <- total_row %>%
  mutate(
    总计 = paste0(total_count, " (100%)"),
    `基药使用情况` = "合计"
  ) %>%
  dplyr::select(`基药使用情况`, 总计, everything())

# 合并表格
final_df <- bind_rows(wide_df, total_row)

# 行排序
final_df$`基药使用情况` <- factor(final_df$`基药使用情况`, levels = c("0", "1", "合计"))
final_df <- final_df %>% arrange(`基药使用情况`)
...
# 导出Excel
time_str <- format(Sys.time(), "%Y%m%d_%H%M%S")
filename <- paste0("基药统计_含列合计_", time_str, ".xlsx")
write.xlsx(final_df, file = filename, rowNames = FALSE)
message("导出文件：", filename)
...
这个表格的 行是 ATC 编码，列是疾病类型（case_type），单元格是该 ATC 在该疾病中的使用比例，即：

比例=该疾病中该ATC出现次数 / 该疾病中所有ATC出现总次数
...
# 1. 提取 antibiotic_1 和 antibiotic_2 的 case_type 与 ATC
abx_1 <- prescription_rate_data %>%
...
# 8. 输出表格
atc_case_wide %>%
  knitr::kable(caption = "ATC 编码在各疾病类型中的数量与比例（count (proportion)）") %>%
...
# 4. 转为宽格式（行是 ATC，列是 case_type，值为 count）
...
# 5. 输出表格（可视化预览）
atc_case_wide %>%
  knitr::kable(caption = "ATC 编码在各疾病类型中的开药次数") %>%
  kableExtra::kable_styling(full_width = TRUE)

# 6. 生成带日期的文件名并导出为 Excel
...
atc_colors <- qualitative_hcl(
  n = length(atc_list),
  palette = "Dark 3",     # 强调深色系
  l = 40,                 # 亮度（l越小越深），默认是 60-80，我们降到 40
  c = 100                 # 色彩饱和度，100比较高
)
...
prescription_rate_data <- prescription_rate_data %>%
  left_join(
    qualityresult %>% dplyr::select(record_id, `药事服务满意度`),
    by = "record_id"
  )
...
table(prescription_rate_data$`药事服务满意度`, useNA = "always")
...
table_satisfaction <- prescription_rate_data %>%
  filter(!is.na(`药事服务满意度`), !is.na(antibiotic_prescribed)) %>%
  count(antibiotic_prescribed, `药事服务满意度`) %>%
...
table_satisfaction_wide <- table_satisfaction %>%
  ...
  pivot_wider(names_from = `药事服务满意度`, values_from = n_percent)
...
# 徐老师建议文章
## 抗生素是否是唯一的药，还是有其他联用的药 
文档见 `output_by_case_type.xlsx`
...
process_sheet <- function(sheet_name) {
  ...
  if (length(missing_cols) > 0) {
    stop(paste("Sheet", sheet_name, "缺少列:", paste(missing_cols, collapse = ", ")))
  }
  
  # 只保留目标列，转换 drug_total_fee 为数值型（若转换失败则为NA）
  df <- df %>%
    dplyr::select(record_id, drug_total_fee, common_name) %>%
    mutate(drug_total_fee = as.numeric(drug_total_fee))
  
  # 费用聚合：每个record_id取第一个非NA的drug_total_fee，如果全NA则为NA
  fee_data <- df %>%
    group_by(record_id) %>%
    dplyr::summarize(
...
# 假设 final_data 已经加载且有 record_id 列

filtered_data <- combined_drug_data %>%
  semi_join(final_data %>% dplyr::select(record_id), by = "record_id")

# 查看结果
print(nrow(filtered_data))  # 应该是2143条左右


# combined_drug_data 是你合并的药品数据
# final_data 是你已有的参考数据框，里面有 record_id 列
...
abx_plot <- ggplot(abx_prop, aes(x = prop, y = fct_rev(case_type), fill = ATC)) +
  ...
  theme_minimal(base_size = 36) +  # 基础字体更大
  theme(
    legend.position = "bottom",
...
mediation_clean <- mediation_clean %>%
  mutate(
    case_type = factor(case_type)  # 转为因子即可
  )
...
## 第一个M1 history_taking_ratio，M2是diagnosis_quality——跑出来发现多分类没法放入
```{r}
...
外生协变量时如何处理编码，比如`case_type`, `province`，`sp_age_group`,`doctor_age`等无序或者三等级制有序变量：

如果是分类变量，建议拆成虚拟变量（dummy variables），防止lavaan因为多类别非序变量报错。

例如：slide有3类，拆成slide_1、slide_2两个0/1变量（参考类不建变量）。




## 第一个M1 history_taking_ratio，M2是diagnosis_quality——开始进行哑变量处理
```{r}


library(fastDummies)

# 创建 dummy 变量（不保留原始变量，且去掉每个分类的参考组，避免共线性）
...
mediation_clean_dummies <- fastDummies::dummy_cols(
  ...
  remove_first_dummy = TRUE,      # 删除参考组，避免共线性
  remove_selected_columns = TRUE  # 移除原始变量
)


mediation_clean_dummies <- dummy_cols(
  mediation_clean_dummies, # 你之前已经包含其他 dummy 的数据
  select_columns = "diagnosis_quality_clean",
  remove_first_dummy = TRUE,   # 移除参考组，避免多重共线性
  remove_selected_columns = TRUE  # 删除原变量
)
...
multipleMediation <- '
  antibiotic_prescribed ~ 
      case_type_偏头痛 + case_type_压力性尿失禁 + case_type_哮喘 + case_type_小儿腹泻 + case_type_心绞痛 +
      case_type_普通感冒 + case_type_糖尿病 + case_type_胃炎 + case_type_腰背痛 + case_type_高血压 +
      ...
      `sp_age_group_30-49` + `sp_age_group_≥50` + 
      `doctor_age_30-50` + `doctor_age_≥50` + 
      doctor_ethnic +
      b1*history_taking_ratio + 
      b2a*diagnosis_quality_clean_部分正确 + 
      b2b*diagnosis_quality_clean_错误 + 
...
## 第二个M1 physical_exam_ratio，M2是diagnosis_quality——开始进行哑变量处理
...
## 第三个M1 lab_imaging_ratio，M2是diagnosis_quality——开始进行哑变量处理
...
## 第四个M1 diagnosis_quality——单一中介

```{r}

simpleMediation <- '
  # 路径：consultation_time -> diagnosis_quality -> antibiotic_prescribed
  antibiotic_prescribed ~ 
      b2a*diagnosis_quality_clean_部分正确 + 
      b2b*diagnosis_quality_clean_错误 +
      c1*consultation_time +
  ...
  # 中介方程
  diagnosis_quality_clean_部分正确 ~ a2a*consultation_time +
      case_type_偏头痛 + case_type_压力性尿失禁 + case_type_哮喘 + case_type_小儿腹泻 + case_type_心绞痛 +
      case_type_普通感冒 + case_type_糖尿病 + case_type_胃炎 + case_type_腰背痛 + case_type_高血压 +
      province_Guangdong + province_Guizhou + province_Hunan + province_InnerMongolia + province_Shaanx + province_Sichuan +
      doctor_female + management_type + ownership_type + rural_type + sp_gender + 
  ...
  diagnosis_quality_clean_错误 ~ a2b*consultation_time +
      case_type_偏头痛 + case_type_压力性尿失禁 + case_type_哮喘 + case_type_小儿腹泻 + case_type_心绞痛 +
      case_type_普通感冒 + case_type_糖尿病 + case_type_胃炎 + case_type_腰背痛 + case_type_高血压 +
      province_Guangdong + province_Guizhou + province_Hunan + province_InnerMongolia + province_Shaanx + province_Sichuan +
      doctor_female + management_type + ownership_type + rural_type + sp_gender + 
...
rate_ci <- result %>%
  ...
  mutate(
    ...
    antibiotic_rate = paste0(round(rate, 2), "% (", ci_low, "–", ci_high, "%)")
  ) %>%
...
result <- formal_data_final_modeimputed %>%
  ...
  mutate(
    ...
    prescription_rate_ci = paste0(prescription_rate_percent, "% (", ci_low, "–", ci_high, "%)")
  ) %>%
  ungroup() %>%
  select(
    case_type,
...
result <- formal_data_final_modeimputed %>%
  mutate(
    institution_type_grouped = case_when(
      institution_type_houyu %in% c("乡镇卫生院", "村卫生室") ~ "乡镇卫生院/村卫生室",
